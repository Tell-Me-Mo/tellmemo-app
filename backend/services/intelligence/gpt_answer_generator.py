        ...
        try:
            ...
            try:
                ...
                if "answer" not in result:
                    ...
                    logger.debug(f"GPT response keys: {list(result.keys())}")
                    return None
                ...
                if confidence_raw is None:
                    logger.warning("GPT response missing 'confidence' field - defaulting to 0.0")
                    logger.debug(f"GPT response: {result}")
                    confidence = 0.0
                elif isinstance(confidence_raw, str):
                    # Handle string confidence values (e.g., "85")
                    try:
                        confidence = float(confidence_raw)
                    except ValueError:
                        logger.warning(f"Invalid confidence string: {confidence_raw} - defaulting to 0.0")
                        confidence = 0.0
                else:
                    confidence = float(confidence_raw)

                # Normalize confidence to 0-1 range (GPT returns 0-100, we use 0-1)
                if confidence > 1.0:
                ...
                # Extract disclaimer with fallback
                ...
                logger.debug(
                    f"Parsed GPT answer: confidence={confidence:.2f} (raw={confidence_raw}), "
...
REQUIRED JSON format (all fields mandatory):
{
  ...
  "confidence": 85,
  "disclaimer": "This answer was generated by AI based on general knowledge, not from your documents or meeting."
}

Confidence scoring guidelines:
- 90-100: Factual, verifiable information (e.g., "What is Kubernetes?")
- 75-89: General knowledge with some uncertainty
- Below 75: Don't answer - insufficient information or too company-specific

IMPORTANT:
- Do NOT fabricate specific company data or internal information
- For company-specific questions (budgets, metrics, processes): Set confidence below 75 to skip answering
